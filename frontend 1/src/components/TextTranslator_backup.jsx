...
const TextTranslator = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isTranslateMode) {
        ...
        //dont forget to replace it back with const translationResponse = await translatePdf(formData);
        //and remove MOCK_RESPONSE={...} as well
        const translationResponse = MOCK_RESPONSE;
        //const translationResponse = await translatePdf(formData);
        ...
        setFileName(translationResult.output_file_name || translationResponse.fileName ||"");
  ...
  const isAiMatchingRunning = loading && progress <= 35;
  const isAiMatchingComplete = progress > 35 || !!outputFileUrl;
  const isLayoutRunning = loading && progress > 35 && progress <= 65;
  const isLayoutComplete = progress > 65 || !!outputFileUrl;
  const isPdfUpdateRunning = loading && progress > 65 && !outputFileUrl;
  const isPdfUpdateComplete = !!outputFileUrl;
    ...
    <div className="translator-container">
      <div className="translator-card ai-shell">
        <div className="hero-panel">
          ...
          <div className="hero-right">
            ...
            <div className="hero-stat-card">
              ...
              <strong>{outputFileUrl ? "Ready" : "Pending"}</strong>
            </div>
          </div>
        </div>
        ...
        <div className="workflow-ribbon">
          <div className={`workflow-node ${selectedFile ? "node-complete" : ""}`}>
            <div className="node-icon">📄</div>
            <div>
              <h4>Upload File</h4>
              <p>Intake source file</p>
            </div>
          </div>

          <div className={`workflow-node ${isAiMatchingRunning ? "node-running" : isAiMatchingComplete ? "node-complete" : ""}`}>
            <div className="node-icon">{isTranslateMode ? "🌐" : "🧠"}</div>
            <div>
              <h4>{isTranslateMode ? "Extract Text" : "AI Content Matching"}</h4>
              <p>{isTranslateMode ? "Read PDF layout JSON" : "Map requested edits"}</p>
            </div>
          </div>

          <div className={`workflow-node ${isLayoutRunning ? "node-running" : isLayoutComplete ? "node-complete" : ""}`}>
            <div className="node-icon">{isTranslateMode ? "🧠" : "📐"}</div>
            <div>
              <h4>{isTranslateMode ? "LLM Translation" : "Layout Analysis"}</h4>
              <p>{isTranslateMode ? "English to Spanish" : "Check spacing and fit"}</p>
            </div>
          </div>

          <div className={`workflow-node ${isPdfUpdateRunning ? "node-running" : isPdfUpdateComplete ? "node-complete" : ""}`}>
            <div className="node-icon">⚙️</div>
            <div>
              <h4>{isTranslateMode ? "PDF Rebuild" : "PDF Update"}</h4>
              <p>{isTranslateMode ? "Preserve layout" : "Apply approved changes"}</p>
            </div>
          </div>

          <div className={`workflow-node ${outputFileUrl ? "node-complete" : ""}`}>
            <div className="node-icon">⬇️</div>
            <div>
              <h4>Download Output</h4>
              <p>Export final PDF</p>
            </div>
          </div>
        </div>
        ...
        <div className="app-layout">
          <div className="main-column">
            <div className="section-card">
              ...
              <div className="upload-box upload-box-modern">
                ...
                {uploadMessage && <p className="upload-success-message">{uploadMessage}</p>}
                {uploadErrorMessage && <p className="upload-error-message">{uploadErrorMessage}</p>}
                {fieldErrors.file && <p className="field-error-message upload-field-error">{fieldErrors.file}</p>}
              </div>
            </div>
                  ...
                  <div className="textarea-with-meta">
                    ...
                    <div className="instruction-meta-row">
                      <p className="word-count-note">
                        Word count: <span className={countWords(userInstructions) >= 200 ? "word-count-limit" : ""}>{countWords(userInstructions)} / 200</span>
                      </p>
                      {fieldErrors.user_instructions && <p className="field-error-message side-error">{fieldErrors.user_instructions}</p>}
                    </div>
                  </div>
                </div>
                ...
                <div className="section-card">
                  ...
                  <div className="instruction-list">
                    <div className="instruction-card modern-instruction-card">
                      <div className="instruction-card-header"></div>
                        ...
                        <div className="instruction-field">
                          ...
                          <div className="field-input-with-error">
                            <select
                              ...
                              onChange={(e) => handleInstructionChange("language", e.target.value)}
                              disabled={loading}
                            >
                        ...
                        <div className="instruction-field">
                          ...
                          <div className="field-input-with-error">
                            <input
                              ...
                              onChange={(e) => handleInstructionChange("page_number", e.target.value)}
                              placeholder="Enter page number"
                              disabled={loading}
                            />
                          </div>
                          <p className="field-error-message">{fieldErrors.page_number || ""}</p>
                        </div>
                        ...
                        <div className="instruction-field full-width">
                          ...
                          <div className="field-input-with-error">
                            <input
                              ...
                              onChange={(e) => handleInstructionChange("section_name", e.target.value)}
                              placeholder="Enter section name"
                              disabled={loading}
                            />
                          </div>
                          <p className="field-error-message">{fieldErrors.section_name || ""}</p>
                        </div>
                        ...
                        <div className="instruction-field full-width">
                          ...
                          <div className="field-input-with-error field-input-with-error-textarea">
                            <textarea
                              ...
                              onChange={(e) => handleInstructionChange("old_text", e.target.value)}
                              placeholder="Enter old text"
                              rows={4}
                              disabled={loading}
                            />
                          </div>
                          <p className="field-error-message">{fieldErrors.old_text || ""}</p>
                        </div>
                        ...
                        <div className="instruction-field full-width">
                          ...
                          <div className="field-input-with-error field-input-with-error-textarea">
                            <textarea
                              ...
                              onChange={(e) => handleInstructionChange("new_text", e.target.value)}
                              placeholder="Enter new text"
                              rows={4}
                              disabled={loading}
                            />
                          </div>
                          <p className="field-error-message">{fieldErrors.new_text || fieldErrors.same_text || ""}</p>
                        </div>
                      </div>
                    </div>
                  </div>
          ...
          <div className="side-column">
            <div className="side-card">
              ...
              <div className="agent-activity-stack">
                <div className="activity-item">
                  <span className={`activity-dot ${selectedFile ? "active-dot" : ""}`} />
                  <div>
                    <strong>Upload PDF</strong>
                    <p>{selectedFile ? "PDF received and validated." : "Waiting for file upload."}</p>
                  </div>
                </div>

                <div className="activity-item">
                  <span className={`activity-dot ${isAiMatchingRunning ? "active-dot pulse-dot" : isAiMatchingComplete ? "active-dot" : ""}`} />
                  <div>
                    <strong>{isTranslateMode ? "Extract Layout JSON" : "AI Content Matching"}</strong>
                    <p>
                      {isAiMatchingRunning
                        ? isTranslateMode
                          ? "Extracting text blocks from PDF JSON..."
                          : "Matching instructions to relevant content..."
                        : isAiMatchingComplete
                        ? isTranslateMode
                          ? "Text extraction completed."
                          : "Content matching completed."
                        : "Will begin after submit."}
                    </p>
                  </div>
                </div>

                <div className="activity-item">
                  <span className={`activity-dot ${isLayoutRunning ? "active-dot pulse-dot" : isLayoutComplete ? "active-dot" : ""}`} />
                  <div>
                    <strong>{isTranslateMode ? "LLM Translation" : "Layout Analysis"}</strong>
                    <p>
                      {isLayoutRunning
                        ? isTranslateMode
                          ? "Translating English text to Spanish..."
                        ...
                        : isTranslateMode
                        ? "Will begin after extraction."
                        : "Will begin after content matching."}
                    </p>
                  </div>
                </div>

                <div className="activity-item">
                  <span className={`activity-dot ${isPdfUpdateRunning ? "active-dot pulse-dot" : isPdfUpdateComplete ? "active-dot" : ""}`} />
                  <div>
                    <strong>{isTranslateMode ? "PDF Reconstruction" : "PDF Update"}</strong>
                    <p>
                      {isPdfUpdateRunning
                        ? isTranslateMode
                          ? "Generating translated PDF with preserved layout..."
                          : "Applying approved modifications..."
                        : isPdfUpdateComplete
                        ? "Output PDF generated."
                        : isTranslateMode
                        ? "Will begin after translation."
                        : "Will begin after layout analysis."}
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="side-card">
              <div className="section-card-header">
                <div>
                  <p className="section-step">Processing</p>
                  <h3 className="section-title">Progress</h3>
                </div>
              </div>

              {loading ? (
                <div className="progress-wrapper progress-wrapper-modern">
                  ...
                  <p className="agent-message">{agentMessages[currentStep]}</p>
                </div>
              ) : (
                <div className="progress-idle-card">
                  <div className="idle-chip">{outputFileUrl ? "Completed" : "Awaiting Run"}</div>
            ...
            <div className="side-card result-section">
              <div className="section-card-header">
                <div>
              ...
              {errorMessage && !outputFileUrl ? (
                ...
                <div className="result-placeholder modern-placeholder">
                  ...
                  <p>Download option will appear here after processing.</p>
                </div>
              ) : (
                <>
                  {successMessage && (
                    <div className="success-banner-card">
                      ...
                      <div>
                        ...
                        <p>{isTranslateMode ? "Your translated PDF is ready for review." : "Your updated PDF is ready for review."}</p>
                      </div>
                    </div>
                  )}
                  ...
                  {summary && (
                    <div className="summary-card">
                      <h4 className="summary-title">{isTranslateMode ? "Translation Summary" : "Update Summary"}</h4>
                      <ul className="summary-list">
                        {summary.processed_file && <li><strong>Processed File:</strong> {summary.processed_file}</li>}
                        {summary.source_language && <li><strong>Source Language:</strong> {summary.source_language}</li>}
                        {summary.target_language && <li><strong>Target Language:</strong> {summary.target_language}</li>}
                        {summary.translated_blocks !== undefined && <li><strong>Translated Blocks:</strong> {summary.translated_blocks}</li>}
                        {summary.total_updates !== undefined && <li><strong>Total Updates:</strong> {summary.total_updates}</li>}
                        {summary.applied_updates !== undefined && <li><strong>Applied Updates:</strong> {summary.applied_updates}</li>}
                        {summary.skipped_updates !== undefined && <li><strong>Skipped Updates:</strong> {summary.skipped_updates}</li>}
                        {isTranslateMode && translationValidation && (
                            <>
                              ...
                              <li>
                                <strong>Validation Passed:</strong> {translationValidation.passed_count}
                              </li>
                              <li>
                                <strong>Validation Failed:</strong> {translationValidation.failed_count}
                              </li>
                            </>
                          )}
                      </ul>
                  ...
                  {isTranslateMode && translationValidation && (
                    <div className="summary-card validation-card">
                      ...
                      <div className="validation-detail-box">
                        {translationValidationDetails.length > 0 ? (
                          ...
                          <p className="no-warning-text">
                            No failed validation blocks reported.
                          </p>
                        )}
                      </div>
                    </div>
                  ...
                  <div className="summary-card warning-card">
                    ...
                    {layoutWarnings.length > 0 ? (
                      <ul className="summary-list">
                        {layoutWarnings.map((warning, index) => <li key={index}>{warning}</li>)}
                      </ul>
                    ) : (
                  ...
                  <div className="download-row centered-download">
                    <button className="download-button" onClick={handleDownload}>
                      {isDownloaded ? "Downloaded" : isTranslateMode ? "Download Translated PDF" : "Download Updated PDF"}
                    </button>
                  </div>

                  {isDownloaded && (
                    <p className="download-success-message">
                      {isTranslateMode ? "Translated PDF downloaded successfully" : "Updated PDF downloaded successfully"}
                    </p>
                  )}
                </>
              )}
