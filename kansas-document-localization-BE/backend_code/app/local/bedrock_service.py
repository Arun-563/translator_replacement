...
class BedrockService:
    ...
    def __init__(self, model_name: str = None):
        ...
        # Use override if provided, otherwise fall back to .env
        self.model_id = model_name or settings.OLLAMA_MODEL_ID
    ...
    def invoke_json_prompt(self, prompt: str) -> dict:
        ...
        try:
            ...
            try:
                json_str = _sanitize_json_output(json_str)
...
def _sanitize_json_output(text: str) -> str:
    """
    Fixes common JSON issues produced by the translation LLM:
    - leading zeros in numbers (e.g. 00.97 -> 0.97)
    - trailing commas
    - smart quotes -> normal quotes
    """

    # Fix leading zeros in numbers like 00.97 → 0.97
    text = re.sub(r'(?<![\d.])0+(\d+\.\d+)', r'\1', text)
    text = re.sub(r'(?<![\d.])0+(\d+)(?=[,\s}])', r'\1', text)
