...
async def process_pdf_translation_request(
    ...
    try:
        ...
        # 6b. Validate translations using a SEPARATE LLM.
        translation_validation_by_block_id = {}
        validation_error = None

        try:
            translation_validation_by_block_id = translation_agent.validate_translations(
                original_text_by_block_id=text_by_block_id,
            ...
            logger.warning(f"Translation validation failed (non-fatal): {e}")


        # 6c. Apply suggested fixes for invalid translations (optional but recommended).
        for block_id, validation in translation_validation_by_block_id.items():
            if not validation.get("is_valid", False):
                suggested_fix = validation.get("suggested_fix", "").strip()
                if suggested_fix:
                    translated_text_by_block_id[block_id] = suggested_fix

        # 6d. Build validation summary.
        validation_failed_blocks = {
            block_id: validation
            for block_id, validation in translation_validation_by_block_id.items()
            if not validation.get("is_valid", False)
        }
        validation_passed_count = (
            len(translation_validation_by_block_id) - len(validation_failed_blocks)
        )
        validation_failed_count = len(validation_failed_blocks)

        # ✅ Compute average validation confidence score
        total_score = 0.0
        score_count = 0

        for validation in translation_validation_by_block_id.values():
            try:
        ...
        average_validation_score = (
            round(total_score / score_count, 4) if score_count > 0 else 0.0
        )
        ...
        result = {
            ...
            "translation_validation": {
                "total_blocks_validated": len(translation_validation_by_block_id),
                "passed_count": validation_passed_count,
                "failed_count": validation_failed_count,
                "average_score": average_validation_score,
                "failed_blocks": validation_failed_blocks,
